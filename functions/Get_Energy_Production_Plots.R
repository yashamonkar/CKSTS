...
get_initiation_plot <- function(Data_Wind,Data_Solar, 
                                Simulations,
  ...
  Sims_Production <- foreach(ns = 1:nsim, .verbose = TRUE) %dopar% {
    ...
    Production <- list()
    for(i in 1:length(start_hours)) {
    Production[[i]] <- get_production_profile(Total_Supply = Total_Prod,
                                              start_date = start_date,
                                              start_day = start_day,
                                              start_hour = start_hours[i],
                                              horizon = horizon,
  ...
  p1 <- ggplot(Sims_Production) +
    geom_violin(mapping = aes(y = Generation, x = Type, group = Type), 
                col = 'white', fill = 'red') +
    geom_point(Data_Production, mapping = aes(x = Type, y = Generation), 
               size = 3) +
...
get_horizon_plot <- function(Data_Wind,Data_Solar,
                             Simulations,
                             start_date, 
                             start_day,
                             start_hour,
