...
get_crosscorrelations(input_data = temp,
                      nam = paste0("\n Simulation Run - ", sel_sim[1]))

temp <- Sims[[sel_sim[2]]]
temp <- as.data.frame(temp) %>% select(n_sel)
get_crosscorrelations(input_data = temp,
                      nam = paste0("\n Simulation Run - ", sel_sim[2]))

temp <- Sims[[sel_sim[3]]]
temp <- as.data.frame(temp) %>% select(n_sel)
get_crosscorrelations(input_data = temp,
                      nam = paste0("\n Simulation Run - ", sel_sim[3]))
...
consolid_df$x <- cut(consolid_df$x, breaks = seq(-1,30,1))
...
plo <- ggplot()+
  scale_x_continuous(limits=c(-1,24.5)) +
  scale_y_continuous(limits=c(1.2*min(consolid_df$y),1.05)) +
...
for(i in 1:9){
  plot(data_quant[i,],MM[i,], main = paste0("Quantile - ",quant[i]*100,"th - ", Data_Type),
       xlab = "Reanalysis Data", ylab = "Mean Simulated ", 
       pch=19, col='red')
  abline(coef = c(0,1), lwd = 2)
}
...
lines(ts_eval, cdf_og, col='red', lwd = 2)
legend('topleft', legend = c("Median Simulation Value", "Reanalysis Data", "5 - 95 Percentile Range"),
       lty = 1, col = c('black','red','grey'), lwd = 3, cex = 1.2)


#______________________________________________________________________________#
#PDF for th entire field.
#COnvert W/sq-m to MWhr/sq-m
t_fac <- 1   #hrs*kms/Mega
...
par(mfrow=c(1,1), mar = c(6,6,4,1))
plot(og_pdf$x, og_pdf$y, type='l',col='red',
     lwd = 2, main = paste0("Simulated PDF for Entire Field - ", Data_Type), 
...
#______________________________________________________________________________#
#Auto-Correlation for the entire field
tx <- rowSums(X)
og_acf <- acf(tx, plot = FALSE)
consolid_sims <- list()
consolid_points <- list()
...
plo <- ggplot()+
  ...
  geom_boxplot(consolid_df, mapping = aes(y = y, x = x+0.5,group = x, fill = Type),
               outlier.shape = NA, outlier.colour = NA)+ 
  ggtitle(paste0("ACF - Aggregated Field - ", Data_Type )) +
  ylab("Auto-Correlation") +
  xlab(paste0("Lags (Hours)")) +
  theme_bw(base_size = 15)
